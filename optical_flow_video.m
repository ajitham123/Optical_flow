...
%  in a video sequence
%  Author: Ajith Anil Meera, 1st August 2017
...
%     [FOE_x FOE_y] = circfit(point(find(flow_mag),1),point(find(flow_mag),2)); 

    % FoE by minimising least square error for the set of optical flow
    % vectors
    is_flow = find(flow_mag);
